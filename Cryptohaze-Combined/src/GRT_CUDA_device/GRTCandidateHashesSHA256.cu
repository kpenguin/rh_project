...
extern "C" void copySHA256CandidateDataToConstant(char *hostCharset, uint32_t hostCharsetLength,
    ...
    CUDA_SAFE_CALL(cudaMemcpyToSymbol("SHA256_Candidate_Device_Charset_Constant", hostCharset, 512));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol("SHA256_Candidate_Device_Charset_Length", &hostCharsetLength, sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol("SHA256_Candidate_Device_Chain_Length", &hostChainLength, sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol("SHA256_Candidate_Device_Table_Index", &hostTableIndex, sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol("SHA256_Candidate_Device_Number_Of_Threads", &hostNumberOfThreads, sizeof(uint32_t)));
}
...
extern "C" void copySHA256HashDataToConstant(unsigned char *hash) {
    ...
    // my life easier.
    CUDA_SAFE_CALL(cudaMemcpyToSymbol("SHA256_Candidate_Device_Hash", hash, 20 * sizeof(unsigned char)));
}
